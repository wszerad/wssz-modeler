...
export function extractDecoratorMarkers(markers: PropMarkers, decorator: Function, defaultValue?: any) {
	if (!markers.has(decorator)) {
		return;
	}
	const marker = markers.get(decorator);
	return (marker !== undefined) ? marker : defaultValue;
}
